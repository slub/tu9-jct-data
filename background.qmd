...
university is represented by the ROR ids listed for its `slug` in
`data-raw/orgs.csv`, including libraries and university hospitals where these
hold separate RORs. The `members_ror` column records which of those ROR ids were
matched for each agreement.
